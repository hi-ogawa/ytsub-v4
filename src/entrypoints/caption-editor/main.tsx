...
function App() {
	...
	>(new Map());
	const [iframeSender, setIframeSender] = React.useState<{
		tabId?: number;
		frameId?: number;
	} | null>(null);
	...
	const sendMessageToIframe = (action: string, payload: any): Promise<any> => {
		return new Promise((resolve, reject) => {
			...
			setPendingRequests((prev) =>
				new Map(prev).set(requestId, { resolve, reject }),
			);
	...
	const testApiAccess = async () => {
		...
		try {
			if (!iframeReady) {
				throw new Error(
					"Iframe content script not ready. Please wait a moment and try again.",
				);
			}
			...
			<main className="main">
				<div className="test-section">
					...
					<div className="status-info">
						<p>
							<strong>Iframe Content Script:</strong>{" "}
							{iframeReady ? "Ready" : "Loading..."}
						</p>
						<p>
							<strong>Iframe Location:</strong>{" "}
							{iframeSender
								? `Tab ${iframeSender.tabId}, Frame ${iframeSender.frameId}`
								: "Not found"}
						</p>
						<p>
							<strong>Pending Requests:</strong> {pendingRequests.size}
						</p>
					</div>
					...
					<div className="input-group">
						...
						<button
							...
							disabled={
								isLoading || !videoId.trim() || !iframeReady || !iframeSender
							}
				...
				<div className="debug-section">
					...
					<p>
						This tests if we can communicate with the YouTube content script in
						the hidden iframe above.
					</p>
				</div>
			</main>
		</div>
